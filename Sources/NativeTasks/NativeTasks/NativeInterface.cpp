...
extern "C"
{
	...
	{
		copy2vuyToLuma(src, dst, width, height);
	}
	...
	{
		copyLumaToColor(src, dst, width, height);
	}
	...
	{
		copyColorToLuma(src, dst, width, height);
	}
	...
	{
		resampleNearestNeighborLuma(src, srcWidth, srcHeight, dst, dstWidth, dstHeight);
	}
	...
	{
		resampleNearestNeighborColor(src, srcWidth, srcHeight, dst, dstWidth, dstHeight);
	}
	...
	{
		resampleLerpFastLuma(src, srcWidth, srcHeight, dst, dstWidth, dstHeight);
	}

	/// Rotates an image by 180-degrees
	///
	/// This is an optimized method to flip the image horizontally and vertically in-place in a single pass
	void nativeRotate180(const NativeLumaBuffer buffer, uint32_t width, uint32_t height)
	{
	...
	/// simply register with `nil`.
	void nativeLogRegisterDebug(NativeLogReceiver receiver) { Logger::registerDebugReceiver(receiver); }
	void nativeLogRegisterInfo(NativeLogReceiver receiver) { Logger::registerInfoReceiver(receiver); }
	void nativeLogRegisterWarn(NativeLogReceiver receiver) { Logger::registerWarnReceiver(receiver); }
	void nativeLogRegisterError(NativeLogReceiver receiver) { Logger::registerErrorReceiver(receiver); }
	void nativeLogRegisterSevere(NativeLogReceiver receiver) { Logger::registerSevereReceiver(receiver); }
	void nativeLogRegisterFatal(NativeLogReceiver receiver) { Logger::registerFatalReceiver(receiver); }
	void nativeLogRegisterTrace(NativeLogReceiver receiver) { Logger::registerTraceReceiver(receiver); }
	void nativeLogRegisterPerf(NativeLogReceiver receiver) { Logger::registerPerfReceiver(receiver); }
	void nativeLogRegisterStatus(NativeLogReceiver receiver) { Logger::registerStatusReceiver(receiver); }
	void nativeLogRegisterFrame(NativeLogReceiver receiver) { Logger::registerFrameReceiver(receiver); }
	void nativeLogRegisterSearch(NativeLogReceiver receiver) { Logger::registerSearchReceiver(receiver); }
	void nativeLogRegisterDecode(NativeLogReceiver receiver) { Logger::registerDecodeReceiver(receiver); }
	void nativeLogRegisterResolve(NativeLogReceiver receiver) { Logger::registerResolveReceiver(receiver); }
	void nativeLogRegisterBadResolve(NativeLogReceiver receiver) { Logger::registerResolveReceiver(receiver); }
	void nativeLogRegisterCorrect(NativeLogReceiver receiver) { Logger::registerCorrectReceiver(receiver); }
	void nativeLogRegisterIncorrect(NativeLogReceiver receiver) { Logger::registerIncorrectReceiver(receiver); }
	void nativeLogRegisterResult(NativeLogReceiver receiver) { Logger::registerResultReceiver(receiver); }
	void nativeLogRegisterBadReport(NativeLogReceiver receiver) { Logger::registerBadReportReceiver(receiver); }
	void nativeLogRegisterNetwork(NativeLogReceiver receiver) { Logger::registerNetworkReceiver(receiver); }
	void nativeLogRegisterNetworkData(NativeLogReceiver receiver) { Logger::registerNetworkDataReceiver(receiver); }
	void nativeLogRegisterVideo(NativeLogReceiver receiver) { Logger::registerVideoReceiver(receiver); }
	void nativeLogRegisterAlways(NativeLogReceiver receiver) { Logger::registerAlwaysReceiver(receiver); }
	...
	const char *nativeVideoCaptureStart(uint32_t frameWidth, uint32_t frameHeight, uint32_t frameRate, NativeCaptureFrameReceiver receiver)
	{
		try
		{
			gVideoCaptureManager.startCapture(frameWidth, frameHeight, frameRate, receiver);
		}
		...
		{
			return "nativeStartCapture: Caught unknown exception";
		}
	...
	const char *nativeVideoCaptureStop()
	{
		try
		{
			gVideoCaptureManager.stopCapture();
		}
		catch(VideoException &ex)
		{
			return ex.what();
		...
		{
			return "nativeStopCapture: Caught unknown exception";
		}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return; }
		return gVideoCaptureManager.circularImageBuffer()->lock();
	}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return; }
		return gVideoCaptureManager.circularImageBuffer()->unlock();
	}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return nullptr; }
		return gVideoCaptureManager.circularImageBuffer()->get();
	}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return nullptr; }
		return gVideoCaptureManager.circularImageBuffer()->peek();
	}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return 0; }
		return gVideoCaptureManager.circularImageBuffer()->count();
	}
	...
	{
		if (!gVideoCaptureManager.circularImageBuffer()) { return 0; }
		return gVideoCaptureManager.circularImageBuffer()->capacity();
	}
